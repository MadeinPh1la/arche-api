# src/arche_api/mcp/schemas/system.py
# Copyright (c) Arche.
...
class SystemHealthResult(BaseModel):
    ...
    status: str = Field(
        ...,
        description="Overall MCP view of Arche health (ok, degraded, down).",
    )
    request_id: str | None = Field(
        default=None,
        description="Underlying Arche X-Request-ID, if a health check call was made.",
    )
...
class SystemMetadataResult(BaseModel):
    ...
    mcp_version: str = Field(..., description="Arche MCP service version.")
    api_version: str = Field(..., description="Underlying Arche HTTP API major version.")
    ...
    max_tickers_per_request: int = Field(
        ...,
        description="Maximum tickers per MCP request (mirrors arche HTTP rules).",
    )
