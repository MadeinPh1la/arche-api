# tests/unit/mcp/test_arche_http_client.py
...
async def test_get_live_quotes_success() -> None:
    route = respx.get("https://api.arche.test/v2/quotes").mock(
    ...
    client = ArcheHTTPClient(settings=FakeSettings())  # type: ignore[arg-type]
...
async def test_get_live_quotes_non_json_response_raises() -> None:
    respx.get("https://api.arche.test/v2/quotes").mock(
        return_value=Response(
            status_code=200,
            content=b"not-json",
    ...
    with pytest.raises(ArcheHTTPError) as excinfo:
...
@pytest.mark.anyio
@respx.mock
async def test_http_error_maps_to_arche_http_error() -> None:
    respx.get("https://api.arche.test/v2/quotes").mock(
        return_value=Response(
            status_code=400,
            json={
...
@pytest.mark.anyio
@respx.mock
async def test_network_error_maps_to_arche_http_error() -> None:
    url = "https://api.arche.test/v2/quotes"
...
def test_to_mcp_error_non_retryable() -> None:
    ...
    mcp_error = ArcheHTTPClient.to_mcp_error(exc)
